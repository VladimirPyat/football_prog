...
- Приватность: до дедлайна прогноз виден только автору (USER и SUPERVISOR — одинаково); ADMIN — полная видимость для support; после дедлайна — всем (UC-8/UC-9). См. `docs/03_user_scenarios.md` §4.
