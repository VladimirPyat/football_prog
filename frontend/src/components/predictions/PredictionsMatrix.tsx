...
export function PredictionsMatrix({
  ...
  return (
    <div className="overflow-x-auto" data-testid="predictions-matrix">
      <table className="border border-gray-200 rounded-lg text-sm w-max max-w-full">
        <thead>
          <tr className="bg-gray-50">
            <th className="px-2 py-2 text-left font-medium text-gray-700 border-b sticky left-0 bg-gray-50 z-10 text-sm">
              Счет
            </th>
            {matches.map((m) => (
              <th
          ...
          <tr className="bg-gray-50">
            <th className="px-2 py-1.5 text-left text-gray-500 border-b text-sm sticky left-0 bg-gray-50">
              {roundTitle}
            </th>
            {matches.map((m) => (
              <th key={`sub-${m.id}`} className="border-b w-10" />
            ))}
          </tr>
        </thead>
        <tbody>
            ...
            return (
              <tr key={entry.user_id} className="border-b border-gray-100">
                ...
                {matches.map((m) => {
                  ...
                  return (
                    <td key={m.id} className="px-0.5 py-1.5 text-center w-10">
